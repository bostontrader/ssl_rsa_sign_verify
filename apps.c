...
#include <openssl/ssl.h> // OPENSSL_malloc
//#include "s_apps.h"
#include "apps.h"
...
int chopup_args(ARGS *arg, char *buf)
{
    ...
    for (p = buf;;) {
        /ss Skip whitespace. ss/
        while (*p && isspace(_UC(*p)))
            p++;
        if (!*p)
            break;

        /ss The start of something good :-) ss/
        ...
        /ss now look for the end of this ss/
        if (quoted) {
            while (*p && *p != c)
                p++;
            *p++ = '\0';
...
int password_callback(char *buf, int bufsiz, int verify, PW_CB_DATA *cb_data)
{
    ...
    //ss We know that there is no previous user data to return to us ss/
...
/ss See if an empty password will do ss/
...
/ss Return error for unknown extensions ss/
#define X509V3_EXT_DEFAULT              0
/ss Print error for unknown extensions ss/
#define X509V3_EXT_ERROR_UNKNOWN        (1L << 16)
/ss ASN1 parse unknown extensions ss/
#define X509V3_EXT_PARSE_UNKNOWN        (2L << 16)
/ss BIO_dump unknown extensions ss/
...
int copy_extensions(X509 *x, X509_REQ *req, int copy_type)
{
    ...
    for (i = 0; i < sk_X509_EXTENSION_num(exts); i++) {
        ...
        /ss Does extension exist? ss/
        if (idx != -1) {
            /ss If normal copy don't override existing extension ss/
            if (copy_type == EXT_COPY_ADD)
                continue;
            /ss Delete all extensions of same type ss/
            do {
...
#ifndef OPENSSL_NO_ENGINE
/ss Try to load an engine in a shareable library ss/
...
#ifndef OPENSSL_NO_ENGINE
    if (e != NULL)
        /ss Free our "structural" reference. ss/
        ENGINE_free(e);
#endif
}
...
X509_NAME *parse_name(const char *cp, long chtype, int canmulti)
{
    ...
    while (*cp) {
        ...
        *bp++ = '\0';
        ...
        /ss If not at EOS (must be + or /), move forward. ss/
        if (*cp)
            ++cp;
...
/ss Get first http URL from a DIST_POINT structure ss/
...
/ss Try to download delta CRL ss/
...
# endif
    ...
    if (err == ERROR_FILE_NOT_FOUND || err == ERROR_PATH_NOT_FOUND)
        ...
        errno = EINVAL;         /ss we could map more codes... ss/
 err:
    ret = -1;
 ok:
...
# else
    if (usertime)
        now = clock();          /ss sum of user and kernel times ss/
    else {
        struct timeval tv;
...
#elif defined(_SC_CLK_TCK)      /ss by means of unistd.h ss/
# include <sys/times.h>
...
BIO *bio_open_owner(const char *filename, int format, int private)
{
...
#ifdef OPENSSL_SYS_VMS
    /ss VMS doesn't have O_BINARY, it just doesn't make sense.  But,
    ...
    ERR_print_errors(bio_err);
    /ss If we have fp, then fdopen took over fd, so don't close both. ss/
    if (fp)
        fclose(fp);
    else if (fd >= 0)
        close(fd);
...
{
    /ss On Windows select only works for sockets, so we simply don't wait  ss/
...
/ss if OPENSSL_SYS_WINDOWS is defined then so is OPENSSL_SYS_MSDOS ss/
...
# if defined(OPENSSL_SYS_WINDOWS)
    ...
    if (!peeked) {
        /ss Probably redirected input? _kbhit() does not work in this case ss/
        if (!feof(stdin)) {
            return 1;
        }
        return 0;
...
/ss Corrupt a signature by modifying final byte ss/
