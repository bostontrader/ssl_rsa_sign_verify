...
char *opt_progname(const char *argv0)
{
    ...
    /ss Strip off trailing nonsense. ss/
    n = strlen(p);
    if (n > 4 &&
        (strcmp(&p[n - 4], ".exe") == 0 || strcmp(&p[n - 4], ".EXE") == 0))
        n -= 4;

    /ss Copy over the name, in lowercase. ss/
    if (n > sizeof(prog) - 1)
        n = sizeof(prog) - 1;
...
char *opt_progname(const char *argv0)
{
    ...
    /ss Find last special character sys:[foo.bar]openssl ss/
    for (p = argv0 + strlen(argv0); --p > argv0;)
        if (*p == ':' || *p == ']' || *p == '>') {
            p++;
            break;
...
char *opt_progname(const char *argv0)
{
    ...
    /ss Could use strchr, but this is like the ones above. ss/
    for (p = argv0 + strlen(argv0); --p > argv0;)
        if (*p == '/') {
            p++;
            break;
...
//#ifndef NDEBUG
        ...
        //ss Make sure options are legit. ss/
        //assert(o->name[0] != '-');
        //assert(o->retval > 0);
        //switch (i) {
        ...
        //ss Make sure there are no duplicates. ss/
...
/ss Print an error message about a failed format parse. ss/
...
/* Parse a cipher name, put it in *EVP_CIPHER; return 0 on failure, else 1. ss/
...
/ss Look through a list of name/value pairs. ss/
...
/ss Parse an int, put it into *result; return 0 on failure, else 1. ss/
...
/ss Parse a long, put it into *result; return 0 on failure, else 1. ss/
...
/ss Parse an intmax_t, put it into *result; return 0 on failure, else 1. ss/
...
/ss Parse a uintmax_t, put it into *result; return 0 on failure, else 1. ss/
...
int opt_verify(int opt, X509_VERIFY_PARAM *vpm)
{
    ...
    switch ((enum range)opt) {
        ...
        case OPT_V_POLICY:
            ...
            break;
        case OPT_V_PURPOSE:
            /ss purpose name -> purpose index ss/
            ...
            /ss purpose index -> purpose object ss/
            xptmp = X509_PURPOSE_get0(i);

            /ss purpose object -> purpose value ss/
...
int opt_next(void)
{
    ...
    //ss If we have --flag=foo, snip it off ss/
...
/* Return the most recent flag. ss/
char *opt_flag(void)
{
    return flag;
}

/ss Return the unknown option. ss/
char *opt_unknown(void)
{
    return dunno;
}

/ss Return the rest of the arguments after parsing flags. ss/
char **opt_rest(void)
{
    return &argv[opt_index];
}

/ss How many items in remaining args? ss/
...
/ss Return a string describing the parameter type. ss/
...
void opt_help(const OPTIONS *list)
{
    ...
    /ss Starts with its own help message? ss/
    standard_prolog = list[0].name != OPT_HELP_STR;

    /ss Find the widest help. ss/
    for (o = list; o->name; o++) {
    ...
    for (o = list; o->name; o++) {
        ...
        /ss Pad out prefix ss/
        memset(start, ' ', sizeof(start) - 1);
        start[sizeof(start) - 1] = '\0';

        if (o->name == OPT_MORE_STR) {
            /ss Continuation of previous line; pad and print. ss/
            start[width] = '\0';
            BIO_printf(bio_err, "%s  %s\n", start, help);
            continue;
        }

        /ss Build up the "-flag [param]" part. ss/
        p = start;
        *p++ = ' ';
        *p++ = '-';
        if (o->name[0])
